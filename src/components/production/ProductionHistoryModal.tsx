...
export const ProductionHistoryModal = ({ record, isOpen, onClose }: ProductionHistoryModalProps) => {
  const { getProductionHistory } = useAdminProduction();
  const [searchTerm, setSearchTerm] = useState('');

  if (!isOpen || !record) return null;

  const history = getProductionHistory(record.id);
  ...
  const getActionColor = (action: string) => {
    switch (action) {
      ...
      case 'add_stock': return 'bg-emerald-100 text-emerald-800';
      ...
      <DialogContent className="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Historial de Producción: {record.productName}
          </DialogTitle>
        </DialogHeader>
