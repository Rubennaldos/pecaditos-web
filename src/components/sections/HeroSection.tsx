...
const HeroSection = () => {
  ...
  useEffect(() => {
    setIsVisible(true);
    const off = onValue(ref(db, "hero"), async (snap) => {
        ...
        {description && (
          <div className="space-y-4 max-w-lg text-center">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              <span className="px-3 py-1 bg-[#f3e5d2] text-[#473729] rounded-full text-sm font-medium">Sin preservantes</span>
              <span className="px-3 py-1 bg-[#e2dbc9] text-[#473729] rounded-full text-sm font-medium">Ingredientes naturales</span>
              <span className="px-3 py-1 bg-[#e6e1d7] text-[#473729] rounded-full text-sm font-medium">Recetas artesanales</span>
            </div>
          </div>
        )}
